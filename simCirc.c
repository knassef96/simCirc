...
int *evaluate(struct circuit mainCirc, int inputArray[]) {
	int w=32 ; int v=32;
	int *outVals;
	outVals = (int *)malloc(w * v * mainCirc.numOutputs/*,sizeof(mainCirc.numOutputs)*/); // hold the output of the gate instances
	
	for(w=0;w<=mainCirc.numOutputs;w++) {
		outVals[w]=0;
	}
		for(v=0;w<=mainCirc.numOutputs;v++) {
		outVals[v]=0;
	}
